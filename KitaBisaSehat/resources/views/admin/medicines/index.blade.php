        ...
        <form action="{{ route('medicines.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div>
                ...
                <select name="status" class="w-full px-4 py-2.5 rounded-xl bg-white/50 border border-white/60 focus:ring-2 outline-none cursor-pointer">
                    ...
                    <option value="unavailable" {{ request('status') == 'unavailable' ? 'selected' : '' }}>Habis / Tidak Tersedia</option>
                </select>
            </div>
            ...
            <div class="md:col-span-4 flex justify-end gap-2 mt-2">
                @if(request()->hasAny(['search', 'status', 'type']))
                    <a href="{{ route('medicines.index') }}" class="px-4 py-2 text-red-500 hover:bg-red-50 rounded-lg font-bold transition-colors">
                        Reset Filter
                    </a>
                @endif
                <button type="submit" class="px-6 py-2 bg-rs-navy text-white rounded-xl font-bold hover:bg-rs-green transition-colors shadow-md">
                    Terapkan Filter
                </button>
            </div>
        </form>
                    ...
                    @forelse($medicines as $med)
                        <tr class="hover:bg-white/30 transition-colors">
                            <td class="p-5">
                                ...
                                <div class="text-sm text-rs-navy/60 mt-1 line-clamp-1" title="{{ $med->description }}">
                                    {{ $med->description }}
                                </div>
                            </td>
                            <td class="p-5 text-center">
                                @if($med->type == 'keras')
                                    <span class="px-3 py-1 bg-red-100 text-red-600 rounded-full text-xs font-bold border border-red-200">
                                        Keras
                                    </span>
                                @else
                                    <span class="px-3 py-1 bg-blue-100 text-blue-600 rounded-full text-xs font-bold border border-blue-200">
                                        Biasa
                                    </span>
                                @endif
                            </td>
