                    ...
                    @forelse($appointments as $apt)
                        <tr class="hover:bg-white/30 transition-colors">
                            <td class="p-4 text-sm text-rs-navy font-medium">
                                {{ \Carbon\Carbon::parse($apt->date)->format('d M Y') }}<br>
                            ...
                            <td class="p-4">
                                @if($apt->status == 'selesai' && $apt->medicalRecord)
                                    <a href="{{ route('medical_records.show', $apt->medicalRecord->id) }}" class="text-xs font-bold text-rs-teal hover:underline">
                                        Lihat Hasil Medis
                                    </a>
                                @else
